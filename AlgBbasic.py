...
def f(X):
    ...
    cost += distance_matrix[X[-1] - 1][X[0] - 1]
    return cost
...
def alg1(size, iterations, probability):
    ...
    for i in range(0, size):
        cities = [i+1 for i in range(num_cities)]
        population = []
    ...
    for i in range(iterations):
        ...
        for i in range(0, len(P)):
            X = choose(P)
            Y = choose(P)
            splitX = random.randint(0, len(X) - 1)
            Z = X[:splitX]
            for elem in Y:
                if elem not in Z:
                    Z.append(elem)
            choice = random.randint(0, probability - 1)
            if probList[choice] == True:
                Z = mutate(Z)
            newP.append(Z)
        P = newP
    costs = []
